...
export class OrdersService implements OnModuleInit{
  ...
  async create(createOrderDto: createOrderDto): Promise<any> {
    ...
    for (const item of items) {
      const lockKey = `sachira.product:${item.productId}:lock`
      // if not exist this lock will create NX means not exist
      const lock = await this.redis.set(lockKey, 'locked', 'EX', 3600*24, 'NX')

      if(!lock) {
        throw new BadRequestException(`product Id: ${item.productId} is being processed. please try again late`)
      }
    }

    // produce order as an event
    this.producer.send({
      topic: `sachira.order.create`,
  ...
  async consumeConfirmedOrders() {
    await this.consumer.subscribe({
      topic: `sachira.inventory.update`,
    }) 
